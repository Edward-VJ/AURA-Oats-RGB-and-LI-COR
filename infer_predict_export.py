...
for d in \
  ...
  experiments_gh_fs \
  experiments_gh_gsw \
  experiments_gh_h2oleaf \
  experiments_gh_multiclass \
  experiments_gh_vpdleaf \
  experiments_gh_vpleaf
do
  echo "=============================================="
  echo "Running inference + plots for $d"
  echo "=============================================="

  python infer_predict_export.py \
    --results-dir "$d" \
    --dataset-root "/media/edward/HDD/Workspace/Resnet/WHOLE/lam" \
    --runs ALL
done
...
def compute_image_stats(image_dir: Path):
    ...
    if not rgb_vals:
        raise RuntimeError(f"No images found to compute stats in {image_dir}")
...
def load_state_flex(model: nn.Module, ckpt_path: Path):
    ...
    # StrSip possible "module." prefixes (DataParallel)
...
def parse_run_tag(run_dir_name: str):
    ...
    Example: \"resnet-lam-top-only-all-days.csv__swin_large_patch4_window12_384__lr5e-05\"
    """
...
def smart_best_run_names(results_dir: Path) -> List[str]:
    """
    From the latest scoreboard in results_dir, pick the best overall_r2 per model
    and build run names of the form:
    ...
    if "overall_r2" not in df.columns:
        raise RuntimeError(f"'overall_r2' column not found in {scoreboard_csv}")

    # For each model, pick row with max overall_r2
    idx = df.groupby("model")["overall_r2"].idxmax()
    best = df.loc[idx]
...
def main():
    ...
    ap.add_argument("--runs", nargs="+", default=["ALL"],
                    help='Run folder names to evaluate (e.g. "X.csv__resnet50__lr1e-04"). Use ALL to auto-select best run per model from scoreboard.')
    ...
    for run_dir in run_dirs:
        ...
        # CSV paths following the training script convention
        csv_train = dataset_root / "training" / f"train_{dataset_name}"
        csv_test  = dataset_root / "test" / f"test_{dataset_name}"
        if not csv_train.exists():
            raise FileNotFoundError(f"Expected train CSV not found: {csv_train}")
        if not csv_test.exists():
            raise FileNotFoundError(f"Expected test CSV not found: {csv_test}")
        ...
        with pd.option_context("display.max_rows", None, "display.width", 140, "display.precision", 6):
