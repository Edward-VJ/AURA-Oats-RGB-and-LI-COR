#!/usr/bin/env python
# train_compare_single_targets.py – runs each target as a single-variable model
# -------------------------------------------------------------------------------
import time
import json
...
BASE_DIR = Path("/media/edward/HDD/Workspace/OAT_PAPER_2/DATA_GB")
...
def main():
    ...
    # Outer loop: run each target individually
    #single_targets = [["gsw"], ["VPleaf"], ["VPDleaf"], ["Fs"], ["Fm'"], ["yield"], ["biomass"]]
    single_targets = [["biomass"]]

    for target_list in single_targets:
        print(f"\n========================")
        print(f"🔹 Starting single-target run: {target_list[0]}")
        print(f"========================")

        for cfg in DATASETS:
            for mode in INPUT_MODES:
                print(f"\n[INFO] Dataset {cfg['name']} – Mode={mode.upper()} – Target={target_list[0]}")
                root = Path(cfg["root"])
                ...
                RESULTS_DIR = Path(f"paper_2_SINGLEVAR_384_GB_{cfg['name']}_{mode.upper()}_{target_list[0]}")
                RESULTS_DIR.mkdir(parents=True, exist_ok=True)

                for mname in MODELS:
                    try:
                        print(f"\n▶ {cfg['name']}__{mode.upper()}__{mname}__{target_list[0]}")
                        tag = f"{cfg['name']}__{mode.upper()}__{mname}__{target_list[0]}"
                        ...
                        row = {"dataset":cfg["name"], "mode":mode, "model":mname, "target":target_list[0], "lr":best_lr, "val_rmse":best_val}
                        r2_vals=[]
    ...
    out_csv = Path(f"scoreboard_SINGLEVAR_{stamp}.csv")
